...
public class transmissionReceiveFile {
	...
	public transmissionReceiveFile(Socket transmissionSocket, String folderPath) {
		try {
			...
			DataOutputStream dos = new DataOutputStream(transmissionSocket.getOutputStream());
			...
			while (fileLength - current > 0) {
				//num = dis.read(buf);
				if (fileLength - current < 1024) {
					num = dis.read(buf, 0, (int) (fileLength - current));
				} else {
					num = dis.read(buf, 0, 1024);
				}
